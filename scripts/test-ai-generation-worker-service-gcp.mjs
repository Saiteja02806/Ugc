...
const serviceName =
  options.serviceName ||
  process.env.GCP_AI_GENERATION_WORKER_SERVICE_NAME?.trim() ||
  "ugc-ai-generation-worker";
const pubsubTopic =
  options.topic ||
  process.env.UGC_AI_GENERATION_PUBSUB_TOPIC?.trim() ||
  "ugc-ai-generation";
const queueName = options.queueName || "ai-generation";
const canaryUserId =
  options.userId ||
  process.env.GCP_AI_GENERATION_WORKER_SERVICE_CANARY_USER_ID?.trim() ||
  "gcp-ai-generation-worker-service-canary";
const canaryProjectId =
  options.canaryProjectId ||
  process.env.GCP_AI_GENERATION_WORKER_SERVICE_CANARY_PROJECT_ID?.trim() ||
  "gcp-ai-generation-worker-service-canary";
...
const serviceCanaryPlan = {
  expectedResult:
    "failed background job with missing generate_image prompt before any AI provider call",
  generationId,
...
if (!options.yes) {
  throw new Error(
    "Refusing to run the AI-generation worker service canary without --yes.",
  );
}
...
console.log(`Created invalid-input AI generation job ${backgroundJob.id}`);
...
try {
  ...
  console.log(`Always-on service consumed invalid-input job ${backgroundJob.id}`);
  console.log(`Final status: ${finalJob.status}`);
  console.log(`Error: ${finalJob.error_message}`);
  console.log("GCP AI-generation worker service invalid-input canary passed");
} catch (error) {
  console.error(
    `GCP AI-generation worker service canary failed for job ${backgroundJob.id}`,
  );
...
function printDryRunPlan(plan) {
  console.log("GCP AI-generation worker service canary dry run");
  console.log(JSON.stringify(plan, null, 2));
  console.log(
    "This would create one invalid generate_image job, publish it to Pub/Sub, and wait for the always-on Cloud Run Service to fail the job before OpenAI, Gemini, or Runway can be called.",
  );
  console.log(
    "Run with --execute --yes only after the ugc-ai-generation-worker Cloud Run Service is Ready.",
  );
}
...
async function assertNoOpenRealAiGenerationJobs() {
  ...
  if (realOpenJobs.length > 0) {
    throw new Error(
      `Refusing to run service canary because ${realOpenJobs.length} real AI-generation job(s) are open.`,
    );
  }
}
...
async function createCanaryBackgroundJob() {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      created_at: now,
      input_json: {
        canary: "gcp-ai-generation-worker-service-invalid-input",
        generationId,
      },
...
async function enqueueCanaryJob(jobId) {
  return pubsub.topic(pubsubTopic).publishMessage({
    attributes: {
      ...
      source: "gcp-ai-generation-worker-service-invalid-input-canary",
    },
    data: Buffer.from(
      JSON.stringify({
        jobId,
...
function assertExpectedCanaryFailure(job) {
  if (job.status === "completed") {
    throw new Error(
      "The invalid-input service canary completed. Stop and inspect before enabling AI generation.",
    );
  }
...
async function cancelOpenCanaryJob(jobId) {
  ...
  const { data, error } = await supabase
    ...
    .update({
      ...
      error_message:
        "Cancelled after failed GCP AI-generation worker service canary.",
      status: "cancelled",
      updated_at: new Date().toISOString(),
    })
    .eq("id", jobId)
