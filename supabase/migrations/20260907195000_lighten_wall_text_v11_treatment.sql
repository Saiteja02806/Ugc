-- V11 preserves Arial Bold 700 and the measured 5-8-line contract while
-- versioning the lighter 3px outline / 0.30 shadow treatment in application
-- and worker renderers. Historical V10 rows remain valid and unchanged.
do $$
declare
  v_constraint_definition text;
  v_constraint_tail constant text := ', false))';
  v_arial_bold_v11_branch constant text := $wall_text_v11$
 or (
  text_content ->> 'layoutVersion' = 'wall-text-overlay-v11'
  ...
  and text_content -> 'finalLayout' ->> 'version' = 'wall-text-final-layout-v7'
