-- The supplied Wall-of-Text reference uses the packaged Arial Bold face at
-- 700. Keep all earlier typography branches valid so existing cards retain
-- their saved line measurements until the normal reflow path replaces them.
do $$
declare
  v_constraint_definition text;
  v_constraint_tail constant text := ', false))';
  v_arial_bold_v10_branch constant text := $wall_text_v10$
 or (
  text_content ->> 'layoutVersion' = 'wall-text-overlay-v10'
  ...
  and text_content -> 'finalLayout' ->> 'version' = 'wall-text-final-layout-v6'
