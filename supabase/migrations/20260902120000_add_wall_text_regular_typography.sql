-- New Wall-of-Text records use Arial Regular at 400. The V3 Arial Bold branch
-- remains valid so previews and videos already produced from historical rows
-- are never rewritten by this release.
do $$
declare
  v_constraint_definition text;
  v_constraint_tail constant text := ', false))';
  v_arial_regular_v8_branch constant text := $wall_text_v8$
 or (
  text_content ->> 'layoutVersion' = 'wall-text-overlay-v8'
  ...
  and text_content -> 'finalLayout' ->> 'version' = 'wall-text-final-layout-v4'
  and text_content -> 'finalLayout' ->> 'fontFamily' = 'Arial'
  and (text_content -> 'finalLayout' ->> 'fontWeight')::integer = 400
