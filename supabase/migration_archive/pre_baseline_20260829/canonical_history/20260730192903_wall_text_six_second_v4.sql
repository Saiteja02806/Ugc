alter table public.overlay_media_assets
  drop constraint if exists overlay_media_assets_placement_analysis_chk,
  add constraint overlay_media_assets_placement_analysis_chk check (
    placement_analysis is null
    or coalesce(
      jsonb_typeof(placement_analysis) = 'object'
      and (
        (
          placement_analysis ->> 'version' = 'wall-text-placement-v1'
          and placement_analysis ->> 'selectedZone' in (
            'top-left',
            'upper-center',
            'middle-left',
            'lower-left'
        ...
        or
        (
          placement_analysis ->> 'version' = 'wall-text-placement-v2'
          and placement_analysis ->> 'selectedZone' in (
            'upper-middle',
            'middle',
            'lower-middle'
      ...
      and jsonb_typeof(placement_analysis -> 'faceBoxes') = 'array'
      and jsonb_typeof(placement_analysis -> 'importantRegions') = 'array'
      and jsonb_typeof(placement_analysis -> 'faceOverlap') = 'number'
      and jsonb_typeof(placement_analysis -> 'contrastScore') = 'number',
      false
    )
  );
...
alter table public.wall_text_creatives
  alter column generator_version
    set default 'business-profile-wall-text-v4',
  drop constraint if exists wall_text_creatives_text_content_chk,
  add constraint wall_text_creatives_text_content_chk check (
    coalesce(
      jsonb_typeof(text_content) = 'object'
      and text_content ->> 'kind' = 'wall_text'
      and (
        (
          text_content ->> 'layoutVersion' = 'wall-text-overlay-v1'
          and jsonb_typeof(text_content -> 'blocks') = 'array'
          and jsonb_array_length(text_content -> 'blocks') between 2 and 4
        )
        or
        (
          text_content ->> 'layoutVersion' = 'wall-text-overlay-v2'
          and jsonb_typeof(text_content -> 'text') = 'string'
          and char_length(trim(text_content ->> 'text')) between 1 and 600
        )
        or
        (
          text_content ->> 'layoutVersion' = 'wall-text-overlay-v3'
          and jsonb_typeof(text_content -> 'segments') = 'array'
          and jsonb_array_length(text_content -> 'segments') between 2 and 3
          and jsonb_typeof(text_content -> 'fullText') = 'string'
          and char_length(trim(text_content ->> 'fullText')) between 1 and 600
        )
        or
        (
          text_content ->> 'layoutVersion' = 'wall-text-overlay-v4'
          and text_content ->> 'pattern' in (
          ...
          and jsonb_array_length(text_content -> 'segments') between 2 and 3
          and jsonb_typeof(text_content -> 'fullText') = 'string'
          and char_length(trim(text_content ->> 'fullText')) between 1 and 300
        )
      ),
      false
    )
...
revoke all on function public.replace_wall_text_creative_copy_v4(
  text,
  uuid,
  integer,
  text,
...
grant execute on function public.replace_wall_text_creative_copy_v4(
  text,
  uuid,
  integer,
  text,
