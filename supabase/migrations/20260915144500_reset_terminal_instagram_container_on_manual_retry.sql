-- A terminal Instagram container cannot be resumed. When the owner explicitly
-- retries a failed target, clear only that inactive terminal container so the
-- next worker job can create a fresh provider operation. This deliberately
-- does not make terminal media failures auto-retryable.
...
BEGIN
  ...
  -- Do not react to an arbitrary target update. The retry path creates this
  -- queued job in the same transaction before moving the target to scheduled.
  IF NOT EXISTS (
    SELECT 1
  ...
  -- Never erase an active claim or a durable provider publication. The normal
  -- terminal-failure path has already released its claim before a retry can
  -- reach this trigger.
