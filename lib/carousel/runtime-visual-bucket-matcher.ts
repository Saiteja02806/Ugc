...
function assetSupportsIntent(
  ...
  if (!Array.isArray(asset.bestForSlideTypes)) {
    return true;
  }

  const slideIntents = asset.bestForSlideTypes.filter(
...
function rankAssets(params: {
  ...
  return [...params.assets].sort((left, right) => {
    const usageDelta = left.usageCount - right.usageCount;
...
function getMatchReason(params: {
  ...
  if (
    params.asset.imageSubjectClass === "object-only" &&
    params.asset.hasHuman === false &&
    params.asset.faceCount === 0 &&
    params.asset.personCount === 0
  ) {
...
function pickBucketAsset(params: {
  ...
  const bucketAssets = params.assets.filter(
    (asset) =>
      asset.visualBucket === params.visualBucketId &&
...
export function selectRuntimeVisualBucketAssets({
  ...
  for (const slide of slides) {
    ...
    let selectedAsset: ReadyCategoryImageAsset | null = null;
    ...
    if (!selectedAsset) {
      const profileBucketAssets = uniqueBucketAssets.filter((asset) =>
        profile.requiredBucketIds.includes(asset.visualBucket as VisualBucketId),
      );
      ...
      if (selectedAsset) {
        selectedBucketId = selectedAsset.visualBucket;
    ...
    selections.push({
      ...
      hasHuman: selectedAsset.hasHuman,
      imageSubjectClass: selectedAsset.imageSubjectClass,
      intent,
