...
export async function GET(request: Request) {
  ...
  if (!supportedRanges.has(requestedDays as InstagramInsightsRangeDays)) {
    return json(
      {
        ok: false,
        message: "Choose a supported Instagram insight date range.",
      },
      400,
    );
  }
  ...
  try {
    ...
    console.error("Failed to load Instagram insights:", error);

    return json(
      {
        ok: false,
        message: "Could not load Instagram insights right now.",
      },
      500,
    );
  }
