...
const approvedBTypographyMigration = readFileSync(
  new URL(
    "../../supabase/migrations/20260908172213_apply_wall_text_b_52px_typography_v13.sql",
    import.meta.url,
  ),
  "utf8",
);
...
test("rejects the reported overflow and synchronizes the measured font fit", () => {
  ...
  const script = `
    ...
    let forcedWrongSizeRejected = false;
    try {
      await validation.validateWallTextRenderFit(forcedWrongSize);
    } catch (error) {
      forcedWrongSizeRejected = error?.code === validation.WALL_TEXT_RENDER_FIT_REJECTED;
    }
  ...
  const result = JSON.parse(output) as {
    ...
    lines: string[];
    rejected: boolean;
    forcedWrongSizeRejected: boolean;
  };
...
test("uses the general word budget for a compact natural Wall message", () => {
  ...
  const original =
    "After a rushed dinner, tracking every ingredient feels like one chore too many when the day has already felt full, especially when the kitchen is still waiting and tomorrow is already on your mind.";
  const script = `
  ...
  `;
  assert.equal(original.split(/\s+/u).length, 34);
  const output = execFileSync(
    process.execPath,
    [
      "--import",
...
test("V13 keeps every word in one measured 5-8 line block", () => {
  const loaderPath = new URL(
    "../../scripts/next-server-only-test-loader.mjs",
    import.meta.url,
  ).href;
  ...
  assert.equal(result.content.finalLayout.version, "wall-text-final-layout-v9");
