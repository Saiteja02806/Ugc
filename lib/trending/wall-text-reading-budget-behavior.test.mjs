...
test("six-second and longer videos have the same layout-based word allowance", async () => {
  ...
  assert.equal(expected.maxWords, 36);
});

test("a six-second job with an old 16-word cap accepts 36 words without reading-time rejection", async () => {
  ...
  globalThis.fetch = async (_url, init) => {
    ...
    const content = writer
      ? { ideas: [{ candidateIndex: 0, text: "I write a small note at the end of each day so I can see what I did and what I left for later. The list is still there when I wake up again each morning." }] }
  ...
  try {
    ...
    assert.equal(candidate.requiredWordRange.maximum, 36);
    assert.equal(candidate.durationSeconds, undefined);
    assert.equal(result[0].maxWords, 36);
    assert.equal(result[0].content.fullText.split(/\s+/u).length, 36);
