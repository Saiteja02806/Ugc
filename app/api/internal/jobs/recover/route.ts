...
import {
  ...
  reserveMissingInitialTrendingHookGenerationChunks,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const hookInitialRunRepair =
      trendingMissing.length > 0
        ? {
            error: "Trending Hook initial-run repair is not configured.",
            repaired: [],
          }
        : {
            repaired: await reserveMissingInitialTrendingHookGenerationChunks({
              limit: Math.min(limit, 25),
            }),
          };
