...
export function createCreateContentWallTextContent(
  ...
  if (lines.length < CREATE_CONTENT_WALL_TEXT_LINE_RANGE.min) {
    throw new Error(
      "Wall-of-Text needs at least five readable lines before it can be scheduled.",
    );
  }
...
export function getCreateContentWallTextLines(text: string): string[] {
  ...
  if (
    explicitLines.length >= CREATE_CONTENT_WALL_TEXT_LINE_RANGE.min &&
    explicitLines.length <= CREATE_CONTENT_WALL_TEXT_LINE_RANGE.max
  ) {
  ...
  const lineCount = Math.min(
    CREATE_CONTENT_WALL_TEXT_LINE_RANGE.max,
    Math.max(
      CREATE_CONTENT_WALL_TEXT_LINE_RANGE.min,
