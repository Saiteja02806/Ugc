-- V13 is the approved B treatment: 52px Arial Bold, a 4px black outline,
-- 1.10 line-height, normal tracking, and no shadow. Earlier rows retain their
-- own recorded final-layout versions and remain valid under this constraint.
do $$
declare
  v_constraint_definition text;
  v_constraint_tail constant text := ', false))';
  v_arial_bold_v13_branch constant text := $wall_text_v13$
 or (
  text_content ->> 'layoutVersion' = 'wall-text-overlay-v13'
  ...
  and text_content -> 'finalLayout' ->> 'version' = 'wall-text-final-layout-v9'
  and text_content -> 'finalLayout' ->> 'fontFamily' = 'Arial'
  and (text_content -> 'finalLayout' ->> 'fontWeight')::integer = 700
  and (text_content -> 'finalLayout' ->> 'fontSizePx')::integer = 52
