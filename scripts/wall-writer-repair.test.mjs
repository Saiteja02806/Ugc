...
test('a rejected candidate gets a targeted rewrite and already accepted items are not regenerated', async () => {
  ...
  const requests=[];
  const accepted=[];
  const good='My notes looked clear until the day got busy. One list helped me see the small tasks I still needed to finish before dinner.';
  const repair='A full week hides small tasks. One clear plan brings them back so I can see what needs my time before dinner without guessing.';
