alter table public.overlay_media_assets
  add column if not exists placement_analysis jsonb;
...
alter table public.overlay_media_assets
  ...
  add constraint overlay_media_assets_placement_analysis_chk check (
    ...
    or coalesce(
      ...
      and placement_analysis ->> 'version' = 'wall-text-placement-v1'
...
alter table public.wall_text_creatives
  alter column generator_version
    set default 'business-profile-wall-text-v3',
...
revoke all on function public.replace_wall_text_creative_copy_v3(
  text,
  uuid,
  integer,
  text,
...
grant execute on function public.replace_wall_text_creative_copy_v3(
  text,
  uuid,
  integer,
  text,
