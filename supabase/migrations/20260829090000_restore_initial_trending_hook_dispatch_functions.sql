-- Production recorded the earlier 20260828162000 migration version during a
-- historical migration collision, but the two functions below were not
-- installed. Reapply them under a new forward-only version so new Hook runs
-- cannot be committed without their first reserved chunk and durable dispatch
-- record, and existing queued runs with no chunk can be repaired.
...
-- Reserving the first chunk invokes the existing outbox trigger, so normal
-- dispatch recovery can safely create and attach the physical worker job.
