...
const projectId =
  ...
  "ugcsaas";
const topicName = process.env.UGC_CAROUSEL_PUBSUB_TOPIC?.trim() || "ugc-carousel";
...
console.log(`Carousel GCP canary: ${carouselId}`);
...
if (result.backgroundJob.status !== "completed") {
  ...
  result.slides.some(
    (slide) =>
      ...
      !slide.rendered_url.startsWith(expectedStorageBaseUrl),
  )
) {
  console.error(JSON.stringify(result, null, 2));
  throw new Error("GCP Carousel worker completed without a full ready GCS slide set.");
} else {
  const artifact = await saveFreshCarouselArtifact(result);

  console.log(
    `Carousel GCP smoke test passed with ${result.slides.length} rendered slides.`,
  );
...
async function findCanaryTemplate() {
  ...
  throw new Error(
    "No completed test carousel has enough ready category assets for a GCP smoke test.",
  );
}
...
async function createCanaryGeneration(template) {
  ...
  if (error || !data?.id) {
    throw new Error(
      `Could not create GCP carousel canary: ${error?.message ?? "missing id"}`,
    );
  }
...
async function pollCarousel(backgroundJobId, carouselId) {
  ...
  throw new Error("Timed out waiting for GCP carousel generation.");
}

async function saveFreshCarouselArtifact(result) {
  const outputDir = resolve(".tmp", "carousel-gcp-e2e", result.carousel.id);
  const files = [];
  ...
  for (const slide of result.slides) {
    ...
    if (!response.ok) {
      throw new Error(
        `Could not download fresh GCP slide ${slide.slide_number}: ${response.status}`,
      );
    }
