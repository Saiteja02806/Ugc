...
// production box this leaves a 750px physical inner fence.
export const WALL_TEXT_INLINE_SAFE_PADDING = 15;
// Keep this in sync with the application layout gate. A centered, outlined
// SVG line can round onto a fence edge even when its measured width fits, so
// preflight reserves one transparent pixel on each side.
export const WALL_TEXT_RASTER_EDGE_GUARD = 2;

export function getWallTextRasterSafeLineWidth(textBoxWidth: number) {
  return Math.max(
    0,
    textBoxWidth -
...
export function buildWallTextOverlaySvg(params: {
  ...
  const isAvenirNextDemiBold =
    params.content.finalLayout?.version === "wall-text-final-layout-v5";
  const fontFamily =
...
function normalizeFinalLayout(
  ...
  if (
    ...
    (value.version === "wall-text-final-layout-v9" || value.version === "wall-text-final-layout-v8" || value.version === "wall-text-final-layout-v7" || value.version === "wall-text-final-layout-v6"
      ...
      : value.version === "wall-text-final-layout-v4"
        ? value.fontFamily !== "Arial" || value.fontWeight !== WALL_TEXT_ARIAL_REGULAR_FONT_WEIGHT
  ...
  const normalized =
    value.version === "wall-text-final-layout-v9"
      ...
      : value.version === "wall-text-final-layout-v6"
      ? {
          ...normalizedBase,
          fontFamily: "Arial" as const,
          fontWeight: WALL_TEXT_ARIAL_BOLD_FONT_WEIGHT as 700,
