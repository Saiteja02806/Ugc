...
const GENERIC_PHRASE_REWRITES: Array<[RegExp, string]> = [
  ...
  [/take (it|your business|your workflow) to the next level/gi, "make the next step easier to act on"],
...
function truncateText(value: string, maxLength: number) {
  ...
  const shortened = sliced.slice(0, lastSpace > 32 ? lastSpace : sliced.length).trimEnd();
...
function improveWeakPhrase(value: string) {
  ...
  if (lowerValue.includes("centralize and automate work with ai-powered workspace")) {
...
export function buildCarouselSlidePlan(params: {
  ...
  return storyTypes.map((slideType, index) => {
    ...
    if (slideType === "hook") {
      ...
      return {
        ...
        headline: distinctHeadline(
          selectedAngle ||
            pick(carouselAngles, candidateIndex, mainPromise || fallbackHeadlines[0]),
          fallbackHeadlines[0],
        ),
        imageDirection: "Use a clean premium image with open space for a strong hook.",
        listItems: [],
        slideNumber,
        slideType,
        subtext: body,
    ...
    if (slideType === "solution") {
      const body = cleanSupportOrFallback(
        pick(valueProps, sourceIndex + 1, solutionSupportFallback),
    ...
    if (slideType === "benefit") {
      ...
      return {
        ...
        ctaText: null,
        headline: null,
        imageDirection: "Use a confident visual with room for one benefit statement.",
        listItems: [],
        slideNumber,
        slideType,
        subtext: body,
    ...
    if (slideType === "differentiator") {
      const body = cleanSupportOrFallback(
        pick(differentiators, sourceIndex + 1, differentiatorSupportFallback),
      ...
      return {
        ...
        imageDirection: "Show a polished detail that makes the product feel distinct.",
        listItems: [],
        slideNumber,
        slideType,
        subtext: body,
    ...
    return {
      ...
      imageDirection: "End with a clear product-forward visual and simple next step.",
      listItems: [],
      slideNumber,
      slideType,
      subtext: null,
