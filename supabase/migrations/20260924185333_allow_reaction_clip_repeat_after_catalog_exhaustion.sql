-- Reactions rotate through fresh clips first, then repeat the least used
-- existing clips when a user's catalog is exhausted. An active-card
-- reservation remains the default protection, but it must not prevent the
-- final fallback from filling a paid daily quota when every compatible clip
-- is already active.
...
begin
  ...
  -- A single request never repeats a clip. Repetition is only across earlier
  -- cards, after the catalog has been rotated through.
  if exists (
    select 1
  ...
  loop
    ...
    -- Keep the original active-card reservation while another compatible,
    -- unreserved clip remains outside this plan. If every compatible clip is
    -- already active (or is being consumed by this same plan), permit the
    -- selected clip to repeat. The advisory lock above makes this check safe
    ...
    if exists (
      ...
      select 1
      from public.reaction_clip_assets as alternative
      join public.reaction_background_assets as selected_background
        on selected_background.id = item_record.background_asset_id
      where alternative.status = 'active'
        and alternative.has_alpha
        and alternative.foreground_anchor = selected_background.foreground_placement
        and item_record.primary_reaction = any(alternative.reactions)
        and not exists (
          select 1
          from public.user_reaction_assignments as assignment
          join public.reaction_creatives as creative
          ...
          where assignment.user_id = p_user_id
            ...
            and assignment.business_profile_version = run_record.business_profile_version
            and assignment.state = 'active'
            and creative.clip_asset_id = alternative.id
            and creative.render_status in ('queued', 'rendering', 'preview_ready')
        )
        and not exists (
          select 1
          from jsonb_to_recordset(p_items) as planned(clip_asset_id uuid)
          where planned.clip_asset_id = alternative.id
        )
    ) then
...
-- This planner is an internal server/worker operation. Replacing the function
-- preserves its existing behavior but not a safe browser-facing contract.
REVOKE ALL ON FUNCTION public.persist_reaction_generation_plan_v1(
  uuid, uuid, text, jsonb, jsonb
) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.persist_reaction_generation_plan_v1(
  uuid, uuid, text, jsonb, jsonb
