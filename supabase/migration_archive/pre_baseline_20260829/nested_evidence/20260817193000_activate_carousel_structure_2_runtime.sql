...
alter table public.carousel_global_settings
  alter column structure_mode set default 'rotate';

update public.carousel_global_settings
set
  structure_mode = 'rotate',
  structure_config_version = structure_config_version + 1,
  updated_at = timezone('utc', now())
where singleton = true
  and structure_mode = 'structure_1_only'
  and structure_config_version = 1
...
comment on column public.carousel_global_settings.structure_mode is
  'Global owner-controlled structure mode. The runtime activation default is strict batch-level rotation until an owner selects one structure.';
