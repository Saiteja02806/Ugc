-- New Wall-of-Text records use the supplied Avenir Next Demi Bold face at
-- 600. Prior Arial and Inter branches remain valid so existing saved cards
-- keep their original preview and render typography.
do $$
declare
  v_constraint_definition text;
  v_constraint_tail constant text := ', false))';
  v_avenir_next_v9_branch constant text := $wall_text_v9$
 or (
  text_content ->> 'layoutVersion' = 'wall-text-overlay-v9'
  ...
  and text_content -> 'finalLayout' ->> 'version' = 'wall-text-final-layout-v5'
  and text_content -> 'finalLayout' ->> 'fontFamily' = 'Avenir Next'
  and (text_content -> 'finalLayout' ->> 'fontWeight')::integer = 600
  ...
  and generator_version = 'business-profile-wall-text-v9'
