...
test("Reaction brief validation derives the caption from the rendered lines", () => {
  const [brief] = validateReactionBriefBatch({
    briefs: [{
      content: {
        caption: "A stale alternate caption the renderer will not use",
        emotion: "surprise",
        languageFormat: "when",
        lines: ["When the task", "finally makes sense"],
        semantic: {
